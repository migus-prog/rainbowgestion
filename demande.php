...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($type_conge)) {
        $errorMessage[] = "Le type de congé est obligatoire.";
    }
    if (empty($debut)) {
    ...
    if (empty($errorMessage)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO demandes_conge (employe_id, type_conge, date_debut, date_fin, commentaire) VALUES (:employe_id, :type_conge, :debut, :fin, :commentaire)");
            $stmt->execute([
            ...
            // Redirection vers main.php si la demande est un succès
    ...
    <div class="container">
        ...
        <form method="POST" action="demande.php">
            ...
            <select id="type_conge" name="type_conge" required>
                ...
                <option value="maladie" <?php echo ($type_conge == 'maladie') ? 'selected' : ''; ?>>Maladie</option>
                <option value="deces" <?php echo ($type_conge == 'deces') ? 'selected' : ''; ?>>Décès</option>
                <option value="formations" <?php echo ($type_conge == 'formations') ? 'selected' : ''; ?>>Formations</option>
                <option value="missions_etranger" <?php echo ($type_conge == 'missions_etranger') ? 'selected' : ''; ?>>Missions à l'étranger</option>
                <option value="reunion_familiale" <?php echo ($type_conge == 'reunion_familiale') ? 'selected' : ''; ?>>Réunion familiale</option>
                <option value="ceremonies_familiales" <?php echo ($type_conge == 'ceremonies_familiales') ? 'selected' : ''; ?>>Cérémonies familiales</option>
            </select>
